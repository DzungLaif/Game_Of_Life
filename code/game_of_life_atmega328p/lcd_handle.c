...
void _writeData16(uint16_t data) {
 ...
 void ILI9225_setWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
	...
	_startWrite();
	_writeRegister(ILI9225_ENTRY_MODE, 0x1000 | (current_mode << 3) );
...
void ILI9225_drawBitmap_P(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color, uint16_t bg) {
	...
	uint8_t byte=0;	//war vorher nicht initialisiert
	...
	for (j = 0; j < h; j++) {
		for (i = 0; i < w; i++ ) {
			...
			else             ILI9225_drawPixel(x + i, y + j, bg);
		}
	}
}
